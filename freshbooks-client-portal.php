 ...
 * Plugin URI: https://mythicdesigncompany.com/freshbooks-client-portal/
    ...
    <form method="post" action="options.php">
        ...
        <table>
            ...
            if ($err) {
                ...
                $business_memberships = $results["response"]["business_memberships"];
...
function mythic_fcp_authorize_app() {
    if(isset($_GET['code']) && $_GET['state'] == "mythic_fcp_auth") {
        ...
        if ($err) {
            ...
            // Save Bearer Token & Refresh Token in Database
            $results = json_decode($response, true);
                                
            $bearer_token = $results["access_token"];
            $refresh_token = $results["refresh_token"];
            $expiry_time = $results["expires_in"];
...
// Find Client ID On User Registration
...
function mythic_fcp_attach_client_id($login, $user) {
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => "https://api.freshbooks.com/accounting/account/" . get_option('mythic_fcp_account_id') . "/users/clients?per_page=100",
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => "",
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 30,
    ...
    if ($err) {
        ...
        foreach($results["response"]["result"]["clients"] as $client) {
...
function mythic_fcp_show_client() {
    ...
    if(get_user_meta($user_id, 'mythic_fcp_client_id', true)) {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://api.freshbooks.com/accounting/account/" . get_option('mythic_fcp_account_id') . "/users/clients/" . $fcp_client_id . "?include[]=outstanding_balance&include[]=credit_balance&include[]=draft_balance&include[]=overdue_balance&include[]=grand_total_balance",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
        ...
        if ($err) {
            ...
            mythic_fcp_render_client_info($results);
        }
    } else {
...
// Render Client Info
function mythic_fcp_render_client_info($results) {
    $bearer_token = get_option('mythic_fcp_bearer_token');
    
    wp_enqueue_style('mythic_fcp_frontend_css');
    
    $info = $results['response']['result']['client'];
...
function mythic_fcp_render_client_invoices() {
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => "https://api.freshbooks.com/accounting/account/" . get_option('mythic_fcp_account_id') . "/invoices/invoices?include[]=direct_links&per_page=10" . $page . "&search[customerid]=" . $fcp_client_id,
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => "",
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 30,
    ...
    if ($err) {
        ...
        $results = json_decode($response, true);
        $invoices =  $results["response"]["result"]["invoices"];
        ...
        foreach($invoices as $invoice) {
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => "https://api.freshbooks.com/accounting/account/" . get_option('mythic_fcp_account_id') . "/invoices/invoices/" . $id . "/share_link?share_method=share_link",
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => "",
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 30,
            ...
            if ($err) {
                ...
                $results = json_decode($response, true);                
                $link = $results["response"]["result"]["share_link"]["share_link"];
            }
        ...
        if($current_page > 1) {
            ...
            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL => "https://api.freshbooks.com/accounting/account/" . get_option('mythic_fcp_account_id') . "/invoices/invoices?per_page=10&page=" . $prev_page . "&search[customerid]=" . $fcp_client_id,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => "",
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 30,
        ...
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://api.freshbooks.com/accounting/account/" . get_option('mythic_fcp_account_id') . "/invoices/invoices?per_page=10&page=" . $next_page . "&search[customerid]=" . $fcp_client_id,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
